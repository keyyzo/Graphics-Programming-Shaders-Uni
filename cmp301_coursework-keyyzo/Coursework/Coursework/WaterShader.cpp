...
WaterShader::~WaterShader()
{
	...
	// Release the domain sampler state.
	if (sampleStateDomain)
	{
		sampleStateDomain->Release();
		sampleStateDomain = 0;
	}
...
void WaterShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
	...
	renderer->CreateSamplerState(&samplerDesc, &sampleStateDomain);
...
void WaterShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, XMMATRIX viewArr[], XMMATRIX projectionArr[], ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* waterBump, ID3D11ShaderResourceView* waterNormal, ID3D11ShaderResourceView* directionalShadowMap, ID3D11ShaderResourceView* pointMapTextures[],
	ID3D11ShaderResourceView* spotShadowMap, int tessVal, float timeV, float amplitudeV, float frequencyV, float speedV, XMFLOAT3 attenVal, XMFLOAT3 spotAttenVal, Light* waterLight, Light* pointLight, Light* spotLight, float coneVal, float rangeVal)
{
	...
	// set water bump texture to domain shader
	deviceContext->DSSetShaderResources(1, 1, &waterBump);
	deviceContext->DSSetShaderResources(2, 1, &waterNormal);
	deviceContext->DSSetSamplers(0, 1, &sampleStateDomain);
