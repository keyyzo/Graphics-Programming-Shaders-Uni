...
cbuffer HeightMapDisplacementBuffer : register(b1)
{
...
OutputType main(ConstantOutputType input, float2 uvwCoord : SV_DomainLocation, const OutputPatch<InputType, 4> patch)
{
    ...
    // to each cell
    float leftY = texture0.SampleLevel(sampler0, leftTex, 0.0f).r;
    float rightY = texture0.SampleLevel(sampler0, rightTex, 0.0f).r;
    float bottomY = texture0.SampleLevel(sampler0, bottomTex, 0.0f).r;
    float topY = texture0.SampleLevel(sampler0, topTex, 0.0f).r;
    ...
    // of the overall height map
    textureColour = texture0.SampleLevel(sampler0, textureCoords, 0.0f);
    float height = textureColour.y;
    
    // change the y position of each vertex based on the value of the height
    // from the texture and multiplying it to build up the overall height 
    vertexPosition.y = height * displacementVal;
