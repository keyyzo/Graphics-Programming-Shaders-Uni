...
float4 main(InputType input) : SV_TARGET
{
    ...
    if (hasDepthData(directionalProjectedTexCoords))
    {
        ...
        {
            lightColour = saturate(calculateLighting(-lightDirection, input.normal, diffuseColour));
        }
    ...
    finalColours = saturate(lightColour + pointLightColour + spotlightColour + ambientColour) * textureColour;
